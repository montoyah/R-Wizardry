---
title: "R wizardry Homework 5 (week 6), 2017"
output: 
  pdf_document: 
    fig_caption: yes
---
...
This function will take in a vector of numbers and converts them to a vector of z-scaled numbers. You should be able to run "boxplot(zScore(data$biomass))" to recreate figure 1.

```{r fig.cap = "Output of 'boxplot(zScore(data$biomass))'. \\label{figure1}", echo=FALSE}
...
results <- zScore(data$biomass)
par(mfrow = c(1,1))
boxplot(results)
#boxplot(zScore(data$biomass))
...
'simplePlot(x = "day", y = "biomass", data = data, lineCol = 2, lwd = 2, pch = 16, col = 3)' to recreate figure 2, and   
'simplePlot(x = "day", y = "biomass", data = data, lineCol = 2, lwd = 2, pch = 2)' to recreate figure 3 (provided you make sure the parameter names match what you have used).


```{r}
simplePlot = function(x, y, data, lineCol = 2, lwd = 2,...)
{
	plot(data[, y] ~ data[, x], xlab = x, ylab = y,...)
		lines(aggregate(data[, y], list(data[,x]), mean), lwd = lwd, col = lineCol)
}

```

```{r fig.cap = "Output of 'simplePlot(x = 'day', y = 'biomass', data = data, lineCol = 2, lwd = 2, pch = 16, col = 3)'. \\label{figure2}"}
simplePlot(x = "day", y = "biomass", data = data, lineCol = 2, lwd = 2, pch = 16, col = 3)

```

```{r fig.cap = "Output of 'simplePlot(x = 'day', y = 'biomass', data = data, lineCol = 2, lwd = 2, pch = 2)'. \\label{figure3}"}
simplePlot(x = "day", y = "biomass", data = data, lineCol = 2, lwd = 2, pch = 2)
```
...
You should be able to run 
"stackedPlot(x = "day", y1 = "biomass", y2 = "count", data = data, lineCol = 2, lwd = 2, pch = 16, col = 4)" to recreate Figure 4.
...
stackedPlot(x = "day", y1 = "biomass", y2 = "count", data = data, lineCol = 2, lwd = 2, pch = 16, col = 4)
```
