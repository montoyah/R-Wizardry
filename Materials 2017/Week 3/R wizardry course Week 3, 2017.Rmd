...
#Let's read the long-format "Compounds.csv" dataset we saved from last weeek, by reading the RDS files using the readRDS() function. If you didn't save this object, please go to D2L, under content > datsets, and download the file "Compounds_triplicates_long.csv" and read it into R using the read.csv() function. Pick one of the two options.
...
data <- read.csv("/home/oscar/Dropbox/R wizardry/2017/Datasets/Compounds dataset (without staff column).csv")
...
library(dplyr)
early_data2 <- data %>% filter(day < 200)
...
data$Staff <- rep(c("Willy_Wonka","Chuck_Norris"), length(data$day) / 2)

# Alternatively could do that more robustly
data$Staff <- rep(c("Willy_Wonka","Chuck_Norris"),1000)[1:length(data$day)]
head(data)
...
data.frame(data[ ,6], is.na(data[ ,6])) #Is it NA?
...
tapply(data$methane_umoles, data$salinity, FUN=mean, na.rm=TRUE)

tapply(data$methane_umoles, data$group, FUN=sd, na.rm=TRUE)
...
aggregate(data$methane_umoles,list(data$day),mean)

aggregate(data$methane_umoles, list(data$compound, data$group, data$day), na.rm=TRUE, mean)
...
```{r}
aggregate(methane_umoles ~ compound + group, data=data, FUN=mean, na.rm=T)
...
```{r}
head(data)
methane_mean_day <- aggregate(data$methane_umoles, by = list(data$compound,
                                                           data$salinity,
                                                           data$group,
                                                           data$day,
                                                           data$Staff),
...
methane_sd_day <- aggregate(data$methane_umoles, by = list(data$compound,
                                                           data$salinity,
                                                           data$group,
                                                           data$day,
                                                           data$Staff),
...
colnames(data_stats) <- c("compound", "salinity", "group", "day", "staff", "mean_methane", "sd_methane")
head(data_stats)

#Alternatively, add
methane_mean_day$sd <- methane_day_sd$x

methane_mean_day[ , 6] <- methane_day_sd$x #There was no warning about overwritting the methane_mean_day column. Be careful!


#Write the data_stats

#write.csv(data_stats, "compounds_triplicates_long.csv", row.names = FALSE)
```
...
```{r}
#read the data_stats saved file
...
```{r}
ag <- aggregate(methane_umoles ~ day + compound + group, data=data, FUN=median, na.rm=T)
...
for(i in 1:ngenes)
{
  ...
  {
    genome[i,j] <- sample(letters[c(1,3,7,20)],1,replace=TRUE,prob=c(1/4,1/4,1/4,1/4))
  }
