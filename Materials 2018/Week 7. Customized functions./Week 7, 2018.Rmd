...
SE_mean <- function(mydata) #argument "mydata" will be where data of interest should be in
{
...
temp <- data$biomass
set.seed(1)
temp[sample(1:14400,1000)] <- NA # this introduces 1000 random NAs
```
...
#H0: true difference in means is equal to 0 (the to groups differ)
t_tests_results <- t.test(data$biomass[data$nutrient == 'Treatment'],
       data$biomass[data$nutrient == 'Control'])

t_tests_results # p<0.05 so there's statistical difference.


# Access t_test_results object
class(t_tests_results)
str(t_tests_results)
t_tests_results$
...
power <- function(x1, x2, nsamp, draws=1000) #Our default has 1000 bott strapps.
  #x1=treatment, x2=control, nsamp=how many samples at the time, draws= how many "boot strapps"?
{
  boot <- NULL
  for(i in 1:draws)
  {
    ...
    t <- t.test(treat, contr) #or (mean(treat)-mean(contr)) / sqrt( var(treat)/treat + var(contr)/contr )
...
set.seed(10)
power(data$biomass[data$nutrient=="Treatment"],
      data$biomass[data$nutrient=="Control"],
      nsamp=20, draws=10)
