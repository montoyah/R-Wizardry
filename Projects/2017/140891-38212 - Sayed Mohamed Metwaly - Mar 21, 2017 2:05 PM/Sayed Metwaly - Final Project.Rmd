...
### What and Why?
**rMetabolomics** is an R Markdown project that aims to perform data processing and statistical analysis of metabolomics data. Most of the popular statistical software packages perform data procssing/analysis on a single metabolite at a time with limited capability to do batch processing/analysis thus they are of limited benefit when handling metabolomics data. There are some online tools dedicated for the purpose of metabolomics data analysis but issues with internet connectivity and service run down during maintenance times limit their reliability.

### Requirements?
rMetabolomics is fully automated, the only required input is the name of the dataset file. Right now only CSV files are accepted. Dataset must comply with the generally accepted metabolomics data configuration: metabolites in columns, samples in rows and first 2 columns contain sample ID and class respectively. Currently rMetabolomics can process 2 classes only (e.g. disease vs control, survivors vs non-survivors ... etc.).

### How it works?
After entering the dataset file name rMetabolomics will do the following: 

* Data processing 
    + Load dataset and plot raw data.
    
    + Do median fold change normalization and plot normalized data:  
    Typically metabolomics studies samples show high variability in concentration, and the derived metabolic profiles have a heteroscedastic noise structure characterized by increasing variance as a function of increased signal intensity. These sources of experimental and instrumental noise substantially complicate information recovery when statistical tools are used. Median fold change normalization is least compromised by the biologically relevant changes in mixture components and is thus preferable normalization method.
    
    + Do base 10 logarithmic transformation and plot transformed data:  
    Logarithmic transformation of data is usually required given the highly skewed nature of the metabolomics results.

    + Do z scaling and plot scaled data:  
    Data scaling aims to centralize the data based on the mean of each metabolite. It is commonly the final step in data processing before statistical analysis is performed.

* Data analysis
    + Do a correlation matrix (Spearman correlation) for all metabolites and plot the result:  
    Correlation matrix provides an overview of the correlation between all metabolites. Spearman correlation is preferentially selected given the skewed nature of most metabolomics data. 

    + Do a heatmap of the data and plot it:  
    Heatmap provides an intuitive way to evaluate the dataset. First the samples are rearranged based on the sample type/class. The sample type indicator is plotted in the first column of the heatmap. The more a metabolite column shows agreement with the first column (sample type) the more it can be used to separate between sample classes. 
    
    + Do 2 sample t-test comparing between the level of each metabolite in the classes, perform Bonferroni adjustment of p-values for family wise error rate FWER correction, plot significant metabolites and prints a table with the details.
    
    + Calculate required sample size (n) per group for the previously identified significant metabolites:  
    Sample size is calculated using the widely acceptable standards: significance level (alpha) = 0.05, power = 80%.
    
    + Plot an innovative **"Power Array"**, an interactive 3D model which shows the statistical power obtained per each of the significant metabolites at a set of fixed sample sizes (n). Its aim is to help decide the final n to be used in further studies in an easy intuitive way.

### Future plans
Analysis of variance (ANOVA), principal component analysis (PCA), partial least square discriminant analysis (PLS-DA) and orthogonal partial least square discriminant analysis (oPLS-DA) are planned additions in future versions of rMetabolomics. Importantly, next versions will have the capability to automatically decide which is the optimal statistical test to be performed based on the data entered.
...
#-------------------------------------------Function matrixPlot------------------------------------------------------- # Custom function to do boxploting for an entire matrix
...
#-------------------------------------------Function matrixCompare---------------------------------------------------- # Custom function to compare an the mean value of 2 matrices
...
#-------------------------------------------Function drawTable------------------------------------------------------- # Custom function to draw graphic tables
...
#-------------------------------------------Function MDFCN------------------------------------------------------------ # Custom function to perform median fold change normalisation
...
matrixPlot(data_mat=numericData.normalized , title="After MDFC Normalization",x_lab="", y_lab="" , boxplot_orient="v")
...
matrixPlot(data_mat=numericData.log , title="After Log Transformation",x_lab="", y_lab="" , boxplot_orient="v")
...
#Plot z-scaled data
matrixPlot(data_mat=numericData.z , title="After z Scalling",x_lab="", y_lab="" , boxplot_orient="v")

processedDataset <- cbind(rawDataset[,1:2],numericData.z)#saves the final processed data which is composed of first 2 columns of the raw data (sample ID and Class) and processed numeric matrix

```

### Data Processing Summary
Provides visual reprsentation of the data before and after processing.
...
### Statistical Sigificance Test (t-test)
...
### Power Array **(Interactive 3D Model)** 
...
# Recode z-values into color indices
zcol  = cut(dataSubset, nClmns) 
